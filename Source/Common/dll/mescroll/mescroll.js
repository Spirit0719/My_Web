/*
 * 由826343539@qq.com根据mescroll修改而来/*!
...
function MeScroll(a, d) {
  ...
  let b = !!c.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);
  let g = typeof window.orientation == "undefined";
  let e = c.indexOf("Android") > -1 || c.indexOf("Adr") > -1;
...
MeScroll.prototype.initDownScroll = function () {
    ...
    if (d > 0) {
      if (c <= 0) {
        ...
        if (b.optDown.use && !b.inTouchend && !b.isDownScrolling && !b.optDown.isLock && (!b.isUpScrolling || (b.isUpScrolling && b.optUp.isBoth))) {
          let n = Math.abs(b.lastPoint.x - g.x);
          let m = Math.abs(b.lastPoint.y - g.y);
          let l = Math.sqrt(n * n + m * m);
          if (l != 0) {
            let f = Math.asin(m / l) / Math.PI * 180;
          ...
          let j = b.downHight / b.optDown.offset;
          b.optDown.onMoving(b, j, b.downHight)
        }
      }
    } else {
      if (d < 0) {
        let p = b.getScrollHeight();
        let i = b.getClientHeight();
        let h = p - i - c;
  ...
  if (b.optDown.use) {
    ...
    let a = b.optDown.warpId ? b.getDomById(b.optDown.warpId) : b.scrollDom;
    ...
    if (l) {
      if (l.contains("mescroll") || l.contains("mescroll-touch")) {
        ...
        if (l.contains("mescroll-touch-x") || l.contains("mescroll-touch-y")) {
          let b = g.touches ? g.touches[0].pageX : g.clientX;
          let a = g.touches ? g.touches[0].pageY : g.clientY;
          if (!i.preWinX) {
            i.preWinX = b
          }
          if (!i.preWinY) {
          ...
          let k = Math.abs(i.preWinX - b);
          let j = Math.abs(i.preWinY - a);
          let h = Math.sqrt(k * k + j * j);
          i.preWinX = b;
          i.preWinY = a;
          if (h != 0) {
            let f = Math.asin(j / h) / Math.PI * 180;
...
MeScroll.prototype.hideUpScroll = function () {
  ...
  let a = this.upwarp.getElementsByClassName("upwarp-progress")[0];
  if (a) {
...
MeScroll.prototype.scrollTo = function (f, b) {
  let c = this;
  let e = c.getScrollTop();
  let a = f;
  if (a > 0) {
    let d = c.getScrollHeight() - c.getClientHeight();
    if (a > d) {
      a = d
    }
  } else {
